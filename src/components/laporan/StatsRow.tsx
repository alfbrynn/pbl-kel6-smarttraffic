...
export default function StatsRow() {
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        ...
        snap.docs.forEach((doc) => {
          ...
          if (!latest[arah] || d.timestamp_ms > latest[arah].timestamp_ms) latest[arah] = d;
        });
        setDataMap(latest);
      }
    );
  ...
  const totalDeteksi  = rows.reduce((a, d) => a + (d.jumlah_kendaraan ?? 0), 0);
  const rataJarak     = rows.length ? Math.round(rows.reduce((a, d) => a + d.jarak_cm, 0) / rows.length) : 0;
  const jalurNormal   = rows.filter(d => d.status_kepadatan === 'Lancar').length;
  const statusNormal  = rows.length ? Math.round((jalurNormal / rows.length) * 100) : 0;
  const alertCount    = rows.filter(d => d.status_kepadatan === 'Padat' || d.status_kepadatan === 'Cukup Padat').length;
