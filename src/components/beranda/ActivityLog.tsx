...
interface LogItem {
  ...
  waktu: string;
  // Ubah tipe data karena Firestore mengembalikan Object Timestamp
  timestamp_obj: any;
}
...
function generateLogs(docs: any[]): LogItem[] {
  return docs.slice(0, 8).map((doc) => {
    ...
    // Konversi Object Timestamp Firestore ke jam lokal (HH:MM:SS)
    const waktu = d.waktu && d.waktu.toDate
      ? d.waktu.toDate().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' })
      : '--:--:--';
    ...
    if (d.status_lampu === 'HIJAU') {
      ...
      // Tambahkan info sensor IR agar log lebih interaktif
      pesan = `Padat di ${d.jalur_arah || 'jalur'} (Antrean: ${d.jarak_cm}cm, Kendaraan: ${d.jumlah_kendaraan ?? 0})`;
      tipe = 'merah';
    } else if (d.status_kepadatan === 'Cukup Padat' || d.status_kepadatan === 'Antre') {
      pesan = `Kepadatan sedang di ${d.jalur_arah || 'jalur'} (${d.jarak_cm}cm)`;
      tipe = 'kuning';
    } else {
      pesan = `Sensor ${d.sensor_id || 'unknown'} mendeteksi jarak ${d.jarak_cm}cm`;
      tipe = 'info';
    }

    return { id: doc.id, pesan, tipe, waktu, timestamp_obj: d.waktu };
  });
}
...
export default function ActivityLog() {
  ...
  useEffect(() => {
    // Tetap tembak kepadatan_jalan, tapi urutkan berdasarkan field 'waktu'
    const unsub = onSnapshot(
      query(collection(db, 'kepadatan_jalan'), orderBy('waktu', 'desc'), limit(8)),
      (snap) => {
  ...
  // Fallback... (biarkan seperti semula)
  const displayLogs = logs.length > 0 ? logs : [
    { id: '1', pesan: 'Jalur Utara beralih ke HIJAU', tipe: 'hijau' as const, waktu: '14:02:45', timestamp_obj: null },
    { id: '2', pesan: 'Kepadatan terdeteksi di Timur', tipe: 'kuning' as const, waktu: '14:01:22', timestamp_obj: null },
    { id: '3', pesan: 'Sensor SN-SELATAN-02 Sinkronisasi', tipe: 'info' as const, waktu: '13:58:10', timestamp_obj: null },
    { id: '4', pesan: 'Peringatan: Antrean Barat meningkat', tipe: 'merah' as const, waktu: '13:55:04', timestamp_obj: null },
  ];
